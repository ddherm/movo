...
Movo 是一个单用户、多项目的本机 MVP。它面向想快速验证网页产品想法的人：先讨论目标与首版范围，再让编程 Agent 制作原型，最后在手机或电脑上验收。需求讨论、方案确认、代码构建、进度追踪和语音通话都留在同一条项目对话里。手机可以通过私有网络连接运行服务的电脑；项目源码由用户在该电脑上选择存放位置。
...
1. 选择运行服务的电脑上的父文件夹，说出想法，或上传参考截图；Movo 自动创建项目子文件夹。
2. 与 Movo 澄清目标、用户和首版范围；查看方案卡，确认后才启动正式编码。
3. OpenHands 在 Docker 中制作原型。页面显示已完成的阶段、最近动作和需要用户回答的决定。
4. 检查通过后打开预览；继续用文字或语音讨论修改。Agent 遇到需要用户决定的问题时会暂停等待。

## 界面预览

以下是**真实 Movo 前端在桌面和手机尺寸下的截图**，项目名称、路径和对话均为演示数据，不包含私人项目。通话图展示的是未接通的面板；语音能力需要按下文配置后在设备上实测。

### 电脑端

**从想法开始。** 输入文字或添加参考图，选择项目名称与存放位置，再开始需求讨论。

![电脑端新建项目：想法、参考图和存放位置](docs/screenshots/desktop-create.png)

**直接选择本机文件夹。** 浏览运行服务的电脑上的已有父文件夹，主盘或外接盘都可以；项目专用子文件夹由 Movo 创建。

![电脑端文件夹选择器：本机目录与外接磁盘入口](docs/screenshots/desktop-folder.png)

**看见真实进度。** 概览页展示已完成的流程阶段、当前步骤和作品预览入口。

![电脑端项目概览：流程阶段与当前进度](docs/screenshots/desktop-overview.png)

**回看方案与验收结果。** 已确认的方案、最近动作、构建及核心交互检查放在同一个页面。

![电脑端方案卡、预览入口与验收结果](docs/screenshots/desktop-plan.png)

### 手机端

手机访问的是同一项目状态，适合随时查看进度、继续讨论或进入语音通话。

| 查看阶段与预览 | 延续项目对话 | 打开语音通话 |
|---|---|---|
| <img src="docs/screenshots/mobile-overview.png" alt="手机端项目概览与阶段进度" width="250"> | <img src="docs/screenshots/mobile-chat.png" alt="手机端项目对话与语音入口" width="250"> | <img src="docs/screenshots/mobile-voice.png" alt="手机端语音通话面板与插话说明" width="250"> |

语音和文字共用项目上下文。通话时可以开口打断播报，Movo 会听完新的发言后继续回应；当前 MiniMax TTS 语速为 **1.2 倍**。
